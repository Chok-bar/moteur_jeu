...
class GameState:
    ...
    def add_player_to_game(self, player_name: str, game_id: int) -> Tuple[Optional[int], Optional[int]]:
        ...
        for row in range(self.board_size):
            for col in range(self.board_size):
                ...
                if player["role"] == server_pb2.Wolf or distance <= visibility_range:
                    result += game["board"][row][col] if game["board"][row][col] != ' ' else '0'
                else:
                    result += 'X'
                        
        return result
    
    def make_move(self, game_id: int, player_id: int, move_str: str) -> Tuple[bool, Optional[server_pb2.Move]]:
        if game_id not in self.games or player_id not in self.players:
            return False, None
        ...
        if not game["started"] or game["round_in_progress"] == -1 or player["id_game"] != game_id or not player["is_alive"]:
            return False, None
        ...
        if target_cell != ' ':
            target_player_id = self._find_player_at_position(game_id, (new_row, new_col))
            if target_player_id and player["role"] == server_pb2.Wolf and self.players[target_player_id]["role"] == server_pb2.Villager:
                self._kill_player(target_player_id)
            else:
                return False, None
            
        game["board"][curr_row][curr_col] = ' '
        game["board"][new_row][new_col] = 'W' if player["role"] == server_pb2.Wolf else 'V'
    ...
    def _find_player_at_position(self, game_id: int, position: Tuple[int, int]) -> Optional[int]:
        for player_id in self.games[game_id]["players"]:
            if player_id in self.players and self.players[player_id]["position"] == position and self.players[player_id]["is_alive"]:
                return player_id
        return None
...
class GameServerServicer(server_pb2_grpc.GameServerServicer):
    ...
    def Move(self, request, context):
        success, move = self.game_state.make_move(request.id_party, request.id_player, request.move)
    ...
    def CreateGame(self, request, context):
        ...
        return server_pb2.CreateGameResponse(
            status=True,
            id_game=new_game_id
        )
